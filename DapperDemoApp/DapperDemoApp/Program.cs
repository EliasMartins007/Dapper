    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                // 1. Chamar API de usuários
                var service = new UsuarioService();
                string resultado = await service.ObterUsuariosAsync();
                Log.Information("Resultado da API: {Resultado}", resultado);

                // Suponha que o resultado seja um JSON convertível para objeto (opcional)
                // List<Usuario> usuarios = JsonConvert.DeserializeObject<List<Usuario>>(resultado);

                // 2. Gravar produtos com Dapper
                var repository = new ProdutoRepository();
                repository.CriarTabelaSeNaoExistir();

                var produto1 = new Produto { Nome = "API Result - 1", Preco = 199.99m };
                var produto2 = new Produto { Nome = "API Result - 2", Preco = 299.99m };
